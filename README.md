...
The **panel button** at the left of the mail and calendar headers folds the whole left sidebar away, giving the view everything except the ribbon down the far edge; the mail folders stay, since they are the point of the view. The fold belongs to those two tabs: step onto a note and the sidebar comes back, step onto mail or the calendar and it folds again, so the pages are never shut behind a note that has every use for them. Clicking anything on the ribbon brings the sidebar back too, without turning the fold off. The "Focus mode" command does the same if you would rather bind a hotkey. **Nothing else moves your layout**: opening mail or the calendar leaves the workspace exactly as you had it, sidebar panes and all, until you press that button. The sidebar is collapsed through Obsidian's own API rather than hidden with CSS, so the app knows it is shut and its own toggles keep working.
